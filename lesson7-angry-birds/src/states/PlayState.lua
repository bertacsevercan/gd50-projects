...
function PlayState:update(dt)
    if love.keyboard.wasPressed('escape') then
     ...
     -- move the camera based on player movement
    if self.level.launchMarker.alien then
        self.levelTranslateX = -math.max(0, math.min(VIRTUAL_WIDTH, self.level.launchMarker.alien.body:getX() - VIRTUAL_WIDTH / 2))
    end
