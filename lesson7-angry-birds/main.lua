...
- Add a different type of alien for below operation:
- Implement it such that when the player presses the space bar after they’ve launched an Alien 
(and it hasn’t hit anything yet), split the Alien into three Aliens that all behave just like the base Alien.

- Make the obstacle break if the object is fast enough and switch to the broken obstacle, 
after hit again it will be destroyed. 

- Add more levels using a levels_def table.
    levels = {
        [1] = {
            ['aliens'] = {x = "", y = ""}, 
            ['obstacles'] = {x = "", y = ""}
...
-x Make the camera move with the player after thrown.

- Add 3 throw chances, like healths.
 ]]
