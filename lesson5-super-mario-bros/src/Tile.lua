...
function Tile:init(x, y, id, topper, tileset, topperset, water)
    self.x = x
    self.y = y
...
function Tile:render()
    ...
    if self.water then
        local offset = self.topper and 0 or 8
        love.graphics.draw(gTextures['water'], gFrames['water'][self.water + offset],
        (self.x - 1) * TILE_SIZE, (self.y - 1) * TILE_SIZE)
    end
end