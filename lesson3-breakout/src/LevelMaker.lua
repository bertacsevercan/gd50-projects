...
function LevelMaker.createMap(level)
    ...
    local numTotalBricks = numRows * numCols 
    ...
    -- locked treasure brick, 1 per level at random levels
    local lockedBrick =  math.random(5) > 3 and true or false
    local addKeyFlag = false

    local lockedBrickX = math.random(numCols)
    local lockedBrickY = math.random(numRows)
    ...
    for y = 1, numRows do
        ...
        for x = 1, numCols do
            ...
            if skipPattern and skipFlag then
                ...
                -- take out the skipped bricks from total
                numTotalBricks = numTotalBricks - 1
            ...
            if lockedBrick and (x == lockedBrickX and y == lockedBrickY) then
                lockedBrick = false
                addKeyFlag = true
            ...
            local hasPowerup = math.random(2) == 1 and true or false
            local powerupType = math.random(7)
            if hasPowerup and #powerups < math.floor(numTotalBricks / 5) then
                p = Powerup(b.x, b.y, powerupType)
                b.powerup = p
    ...
    if addKeyFlag then
        LevelMaker.createKey(numTotalBricks, bricks, powerups)
    end
...
function LevelMaker.createKey(numTotalBricks, bricks, powerups)
    local index = math.random(numTotalBricks)
    local brick = bricks[index]
    if brick.locked then
        return LevelMaker.createKey(numTotalBricks, bricks, powerups)
    end
    brick.powerup = Powerup(brick.x, brick.y, 8)
    table.insert(powerups, brick.powerup)
end