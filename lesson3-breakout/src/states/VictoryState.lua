...
function VictoryState:enter(params)
    ...
    self.ball = params.ball[1]
    self.recoverPoints = params.recoverPoints
    
    love.filesystem.write('breakout.level.lst', tostring(self.level + 1))
...
function VictoryState:update(dt)
    ...
    local levelMaker = LevelMaker.createMap(self.level + 1)

    -- go to play screen if the player presses Enter
    if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
        gStateMachine:change('serve', {
            level = self.level + 1,
            bricks = levelMaker['bricks'],
            powerups = levelMaker['powerups'],
            paddle = self.paddle,
            health = self.health,
            score = self.score,
