...
-- TODO:
-- Fix recoverPoints bug x
-- Add a powerup to the game that spawns two extra Balls. x
-- Grow and shrink the Paddle when the player gains enough points or loses a life. x
-- Add a locked Brick that will only open when the player collects a second new powerup,
-- a key, which should only spawn when such a Brick exists and randomly as per the Ball powerup. x
-- The song is a banger though!!!
-- Spawn a key for lock if it's lost x
-- Add more powerups in total 8 of them x
-- If there are multiple balls, all balls must be destroyed to lose health x
-- Each brick comes with a possible powerup determined randomly,
-- when a brick breaks it can drop a powerup if it has one. x
-- Show which power up is picked up with a text on screen x
-- Add level save, continue from the last saved level x
...
function love.load()
    ...
    gSounds = {
        ...
        ['powerdown'] = love.audio.newSource('sounds/laser1.wav', 'static'),
...
function loadLevel()
    love.filesystem.setIdentity('breakout.level')
    local level = 1

    if love.filesystem.getInfo('breakout.level.lst') then
        for line in love.filesystem.lines('breakout.level.lst') do
            level = tonumber(line)
        end
    end
    return level
...
function renderPowerupEffect(powerup)
    love.graphics.setFont(gFonts['medium'])
    love.graphics.printf(powerup, 0, VIRTUAL_HEIGHT / 2, VIRTUAL_WIDTH, 'center')
end