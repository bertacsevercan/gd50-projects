...
function Ball:render()
    ...
    if not(gameState == 'play') then
        ...
        -- render the ball red as the speed increases
        normalDx = math.abs(self.dx)
        if normalDx >= 180 then
            if normalDx >= 255 then
                normalDx = 255
            end
            love.graphics.setColor(normalDx/255, 1 - normalDx/255 , 1 - normalDx/255)
        end
    end
