...
function BeginGameState:enter(def)
    ...
    -- max num of rows the board can have
    -- increase it every 3 level
    self.numRows = math.min(8, 4 + math.floor(self.level / 3))

    -- spawn a board and place it toward the right 
    self.board = Board(BOARD_START_X, BOARD_START_Y, self.level, self.numRows)
