...
--[[ TODO:
-x Implement a Menu that appears during the player Pokémon’s level up that shows,
for each stat, ‘X + Y = Z’, where X is the starting stat, Y is the amount
it’s increased for this level, and Z is the resultant sum.
This Menu should appear right after the “Level Up” dialogue that appears
at the end of a victory where the player has indeed leveled up.

Additions:
-x Implement a catch pokemon state.

-x Multiple pokemon in party for battle.

-x Multiple pokemon battle with npc.

-x Put an npc to interact with, dialog and battle.

-x Add a chance to not run away based on the opposition's level.

-x Add random chance to find pokeballs in grass.

-x Use field menu state to show pokemon party
]]
