...
function BattleState:init(player, npc)
    self.player = player
    ...
    self.opponent =  Opponent {
        isNpc = npc and true,
        party = (npc and npc.party) or Party {
            pokemon = {
...
function BattleState:triggerStartingDialogue()
    ...
    local text = 'A wild ' .. tostring(self.opponent.party.pokemon[1].name) .. ' appeared!'
    if self.opponent.isNpc then
        text = 'Opponent sends ' .. tostring(self.opponent.party.pokemon[1].name) .. '!'
    end
    
    -- display a dialogue first for the pokemon that appeared, then the one being sent out
    gStateStack:push(BattleMessageState(text,
