...
TakeTurnState.NAME = "TakeTurnState"
...
function TakeTurnState:init(battleState)
    ...
    self.playerPokemonIndex = self.battleState.playerPokemonIndex or 1
    self.opponentPokemonIndex = self.battleState.opponentPokemonIndex or 1
...
function TakeTurnState:checkDeaths()
    if self.playerPokemon.currentHP <= 0 then
        self.playerPokemonIndex = self.playerPokemonIndex + 1
        self.battleState.playerPokemonIndex = self.playerPokemonIndex

        if self.battleState.player.party.pokemon[self.playerPokemonIndex] then
            self:nextPlayerPokemon()
            return true
        end
        self:faint()
        return true
    elseif self.opponentPokemon.currentHP <= 0 then
        self.opponentPokemonIndex = self.opponentPokemonIndex + 1
        self.battleState.opponentPokemonIndex = self.opponentPokemonIndex

        if self.battleState.opponent.party.pokemon[self.opponentPokemonIndex] then
            self:nextOppPokemon()
            return true
        end
...
function TakeTurnState:nextPlayerPokemon()
     -- drop player sprite down below the window
    Timer.tween(0.2, {
        [self.playerSprite] = {y = VIRTUAL_HEIGHT}
    })
    :finish(function()
        self.battleState.playerPokemon = self.battleState.player.party.pokemon[self.playerPokemonIndex]
        
        -- when finished, push a loss message
        gStateStack:push(BattleMessageState('You fainted!', function()         
            gStateStack:push(BattleNextPlayerState(self.battleState))
        end))
        
    end)
end

function TakeTurnState:nextOppPokemon()
     -- drop player sprite down below the window
    Timer.tween(0.2, {
        [self.opponentSprite] = {y = VIRTUAL_HEIGHT}
    })
    :finish(function()
        self.battleState.opponentPokemon = self.battleState.opponent.party.pokemon[self.opponentPokemonIndex]
        
        -- when finished, push a loss message
        gStateStack:push(BattleMessageState('Opponent fainted!', function()         
            gStateStack:push(BattleNextOppState(self.battleState))
        end))
        
    end)
end

function TakeTurnState:healParty(party)
    for i = 1, #party do
       party[i].currentHP = party[i].HP
    end
end
    ...
    :finish(function()
        ...
        function()
            ...
            function()
                ...
                -- heal party
                self:healParty(self.battleState.player.party.pokemon)
...
function TakeTurnState:victory()
    ...
    :finish(function()
        ...
        -- heal opponent party
        self:healParty(self.battleState.opponent.party.pokemon)
        ...
        function()
            ...
            local exp = #self.battleState.opponent.party.pokemon * ((self.opponentPokemon.HPIV + self.opponentPokemon.attackIV +
            ...
            Timer.after(1.5, function()
                ...
                :finish(function()
                    ...
                    if self.playerPokemon.currentExp > self.playerPokemon.expToLevel then
                        ...
                        local winStats = self.playerPokemon:levelUp()

                        gStateStack:push(BattleMessageState('Congratulations! Level Up!',
                        function() end), false)

                        gStateStack:push(BattleWinMenuState(winStats, 
                        function()
                            gStateStack:pop()
                            self:fadeOutWhite()
                        end))                        
